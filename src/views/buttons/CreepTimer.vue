        ...
        <div class="timer">
          <template>
            ...
            <div class="vote text-center" v-if="section_number">
              <h4>Creep Vote</h4>
              <p>
                Here are this weeks nominations for creeps. Click the one that
            ...
            <div class="nominate" v-else>
              <div class="nom text-center">
                ...
                <p>
                  Enter the exact name of the creep you would like to nominate
                  this round, then come back to vote when the timer runs out.
                  Please note: Nominees must come from the bad list {link to bad
                  list}
                </p>
  ...
  methods: {
    fetchTimer() {
      this.$api.profile.fetch_timer().then((response) => {
        ...
        this.distance =
          this.timePeriodMinutes - (this.diff_seconds % this.timePeriodMinutes);
        this.section_number =
          Math.floor(this.diff_seconds / this.timePeriodMinutes) % 2 === 0
            ? false
            : true;
        ...
        setInterval(() => {
          if (this.distance < 0) {
            ...
            this.hours = Math.floor(this.distance / (1000 * 60 * 60));
